Shader "Custom/Water/ReflCube-Fast2" {
	Properties {
		...
		Pass {
			...
			CGPROGRAM
				...
				fixed4 frag(v2f i) : COLOR {
					...
					float4 bump = tex2D(_WaveTex, i.Uv + speed) + tex2D(_WaveTex, i.Uv - float2(1.5*speed,speed));
					bump *= 0.5;
					//return bump;
					float3 normalW = normalize(UnpackNormal(bump).rbg);
					...
					float3 reflectView = reflect(i.View, normalW);
					float3 reflCol = texCUBE(_CubeTex, reflect(i.View, normalW)).rgb;

					//return float4(reflectView,1);
