...
public class AssetMng : MonoBehaviour
{
    ...
    {
		path = PathTool.SubPathByPlatform(path);
		return PathTool.GetWWWPath(path,pathtype,"");
    ...
    {
			path = PathTool.SubPathByPlatform(path);
			return PathTool.GetWWWPath(path,pathtype,".assetbundle");
    }
	...
	{
		path = PathTool.SubPathByPlatform(path);
		return PathTool.GetFilePath(path,pathtype,"");
	}
                                        ...
                                        bool _isStatic = false)
    {
        ...
        //如果我们要的资源已经加载过了，则直接取（TO DO：这里要对www进行unLoad，目前没找到合适的办法）by邓成
    ...
    /// <summary>
    /// 获取特效源资源（除非非常了解这个接口，否则建议建议使用GetEffectInstance） by邓成
    /// </summary>
    /// <param name="_effctName"></param>
    /// <param name="_callback"></param>
